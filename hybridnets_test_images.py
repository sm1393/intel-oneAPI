...
parser.add_argument('--source', type=str, default='demo/img_benchmarking', help='The demo image folder')
parser.add_argument('--output', type=str, default='demo_result/img_benchmarking', help='Output folder')
...
#     print("input_imgs[0].shape = ", input_imgs[0].shape)
...
with torch.no_grad():
    regressBoxes = BBoxTransform()
    clipBoxes = ClipBoxes()
    for imgCount in range(1, x.shape[0]+1):
        _x = x[0:imgCount]
        t1 = time.time()
        _, regression, classification, anchors, segmentation = model(_x)
        ...
        imgCounts.append(imgCount)
        timeTakenToInfer.append(tact_time)
        print("time", tact_time)